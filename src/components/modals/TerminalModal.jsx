...
import { AboutModal, SkillsModal, ProjectsModal, ContactModal, ProfileModal } from "./index";
import useDraggable from "../../hooks/useDraggable";

const MODALS = {
  about:    { title: "cat about.txt",    Component: AboutModal    },
  profile:  { title: "open profile.webp",  Component: ProfileModal  },
...
export default function TerminalModal({ modalKey, initialPos, isMobile, onClose, zIndex, onFocus }) {
  ...
  const TitleBar = ({ extraRight }) => (
    ...
    >
      ...
      <span className="flex-1 text-center text-xs" style={{ color: C.muted }}>{title}</span>
  ...
  if (isMobile) {
    return (
      ...
      >
        ...
        />
        <div className="overflow-y-auto flex-1 p-5" style={{ color: C.text, fontSize: "0.9rem" }}>
          <Component />
        </div>
      </div>
    );
      ...
      <TitleBar
        extraRight={
          <div className="flex items-center gap-2">
            {zoom && (
              <span className="text-xs px-1 rounded" style={{ color: C.green, background: `${C.green}18` }}>
                ×1.2
              </span>
            )}
            <span className="text-xs" style={{ color: `${C.muted}66` }} title="Drag to move">⠿</span>
          </div>
        }
      />
      <div
