...
/**
 * useDraggable
 * @param {{ initial: {x,y}, enabled: boolean }} opts
 *   enabled=false (mobile) → hook is inert, pos always {0,0}, no listeners attached.
 */
export default function useDraggable({ initial = { x: 0, y: 0 }, enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (!el) { setPos({ x: nextX, y: nextY }); return; }
      ...
      const clampedScreenLeft = Math.min(Math.max(screenLeft, -(rect.width - TITLE_BAR)), window.innerWidth  - TITLE_BAR);
      const clampedScreenTop  = Math.min(Math.max(screenTop,  0),                         window.innerHeight - TITLE_BAR);
