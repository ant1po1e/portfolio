...
const BOOT_LINES = [
    { type: "system", text: "Portfolio OS v2.4.1 — booting..." },
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            if (i < BOOT_LINES.length) {
                ...
                setTimeout(tick, isMobile ? 0 : 220);
            } else {
                setBooted(true);
                setTimeout(() => inputRef.current?.focus(), 50);
            }
        };
        setTimeout(tick, isMobile ? 0 : 300);
        return () => {
            cancelled = true;
        };
    }, []);
            ...
            setOpenModals((prev) => {
                ...
                if (!isMobile) {
                    ...
                    const clampedX = Math.min(
                        Math.max(spawnX, 8),
                        window.innerWidth - MODAL_W - 8,
                    );
                    const clampedY = Math.min(
                        Math.max(spawnY, 40),
                        window.innerHeight - MODAL_H - 8,
                    );
                    setModalInitPos((prev) => ({
                        ...prev,
    ...
    const terminalStyle = isMobile
        ...
        : {
              ...
              transition: isDragging
                  ? "none"
                  : "box-shadow 0.2s, border-color 0.2s, transform 0.15s, height 0.15s",
        ...
        <div
            ...
            {!isMobile && (
                <p
                    ...
                    Drag title bar to move · 🟢 to zoom · type or click
                    shortcuts
                </p>
            )}
